...
def _scalar_to_float(value) -> Optional[float]:
    """Convert plain scalars or OpenMM unit Quantities to a float when possible."""
    if value is None:
        return None
    try:
        return float(value)
        ...
        pass
    try:
        return float(value._value)  # OpenMM Quantity internals; fallback only.
    except Exception:
        return None
