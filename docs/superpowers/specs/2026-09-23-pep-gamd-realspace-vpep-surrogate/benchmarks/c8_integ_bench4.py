...
# none | split (29 + 31, as production) | merged (both in 29) |
# shared (ONE CustomCVForce in 29: the CV2 force's own contact sub-CV also drives the CV1 umbrella,
#         so the 1,256-pair contact sum is evaluated once per step instead of twice)
BIAS = os.environ.get("BIAS", "none")
_cv_args = None
def _add_real_cv_forces(s, bias=None):
    ...
    n_before = s.getNumForces()
    info = add_secondary_structure_cv_force(openmm, s, topology, _cv_args, force_group=29, primary_cv_def=ck["primary_cv"])
    if bias == "shared":
        f = s.getForce(n_before)
        assert isinstance(f, openmm.CustomCVForce) and f.getForceGroup() == 29
        # Same umbrella expression and parameter names as forces.add_contact_umbrella_force,
        # reading the CV2 force's private contact sum (identical switching expression/pairs).
        f.setEnergyFunction(f.getEnergyFunction() + " + 0.5*k*((res_contacts/contact_norm)-r0)^2")
        f.addGlobalParameter("contact_norm", contact_normalization_denominator(list(ck["primary_cv"]["contact_pairs"]), _cv_args))
        f.addGlobalParameter("k", 0.0)
        f.addGlobalParameter("r0", 0.0)
    return info

def make_system(partition: bool, bias=None):
    bias = bias or BIAS
...
def build(i, devices):
    ...
    props = {"Precision": "mixed", "DeviceIndex": devices[i % len(devices)], "DisablePmeStream": os.environ.get("PME_DISABLE", "true"),
    ...
    if ARM == "P5":
        env = json.load(open(f"{RUN}/swarm/analysis/shared_gamd_setup/shared_gamd_setup_globals.json"))["all_globals"]
        for k in range(ig.getNumGlobalVariables()):
            name = ig.getGlobalVariableName(k)
            if name in env:
                ig.setGlobalVariable(k, float(env[name]))
...
if ARM == "CHECK":
    # Equivalence: shared (one force) must give the split pair's bias energy and forces at the
    # same positions and window parameters. Nonzero k/ss_k so every term is live.
    import numpy as np
    WIN = {"k": float(os.environ.get("CHECK_K", "1000.0")), "r0": 0.45, "ss_k": 20.0, "ss0": 0.3}
    out = {}
    for b in ("split", "shared"):
        s = make_system(False, b)
        ig = openmm.VerletIntegrator(1.0 * unit.femtosecond)
        plat = os.environ.get("CHECK_PLATFORM", "CUDA")
        props = {"Precision": "double" if os.environ.get("CHECK_DOUBLE") else "mixed", "DeviceIndex": "0"} if plat == "CUDA" else {}
        ctx = openmm.Context(s, ig, openmm.Platform.getPlatformByName(plat), props)
        ctx.setPeriodicBoxVectors(*equil.getPeriodicBoxVectors())
        ctx.setPositions(equil.getPositions())
        for name, val in WIN.items():
            ctx.setParameter(name, val)
        groups = {29, 31} if b == "split" else {29}
        st = ctx.getState(getEnergy=True, getForces=True, groups=groups)
        out[b] = (st.getPotentialEnergy().value_in_unit(unit.kilojoule_per_mole),
                  np.asarray(st.getForces(asNumpy=True).value_in_unit(unit.kilojoule_per_mole / unit.nanometer)))
        del ctx
    (e1, f1), (e2, f2) = out["split"], out["shared"]
    fmax = float(np.abs(f1).max())
    print(f"RESULT CHECK E_split={e1:.6f} E_shared={e2:.6f} dE={e2-e1:.3e} kJ/mol | "
          f"max|F_split|={fmax:.3f} max|dF|={float(np.abs(f2-f1).max()):.3e} kJ/mol/nm | "
          f"rel={float(np.abs(f2-f1).max())/max(fmax,1e-30):.2e}", flush=True)
    raise SystemExit(0)
...
if ARM == "P5":
    ...
    extra = (f" | stage {ig0.getGlobalVariableByName('stage'):.0f} FSF_T {ig0.getGlobalVariableByName('ForceScalingFactor_Total'):.3f}"
             f" FSF_D {ig0.getGlobalVariableByName('ForceScalingFactor_Dihedral'):.3f}")
print(f"RESULT arm={ARM} bias={BIAS} groups={pep_gamd.pep_gamd_bias_force_groups(sims[0].system)} N={N} steps/rep={chunks*CHUNK} wall={wall:.1f}s | {sps:.2f} steps/s/rep | "
      ...
      f"cpu {cpu/wall:.1f} cores ({cpu/wall/N:.2f}/rep) | E0={e:.0f} kJ/mol finite={e==e} | MPS={bool(os.environ.get('CUDA_MPS_PIPE_DIRECTORY'))} PME_DISABLE={os.environ.get('PME_DISABLE','true')}{extra}", flush=True)
