...
BANK = Path("/run/media/sulcjo/sulcjo-data/IOCB/md/2026_peptide_sampler/chignolin_genpept_rep3_massive/final_implicit_survivor_seeds")
REPORT = Path("/run/media/sulcjo/sulcjo-data/IOCB/md/2026_peptide_sampler/GENPEPT_R7_CV_CENSUS_REPORT")
OUT_CSV = REPORT / "genpept_rep3_cv_census_values.csv"
...
R0_BETA_RESBAL = [(4.5, 6.0), (8.0, 1.5), (8.0, 3.0), (8.0, 6.0), (10.0, 1.5), (10.0, 3.0), (10.0, 6.0),
...
def switch_fraction(d, r0_a, beta_a):
    return 0.5 * (1.0 - np.tanh(0.5 * beta_a * (d - r0_a)))
...
def balanced_weights(top, sets):
    ...
    for name, idxs in sets.items():
        ...
        out[(name, 4)] = (ai, aj, 1.0 / counts[inv])
    return out
...
def main():
    ...
    for (sel, sep), (ii, jj) in plists.items():
        for r0, beta in R0_BETA_CORE:
            contact_jobs.append({"base": f"c_{sel}_s{sep}", "r0": r0, "beta": beta, "ii": ii, "jj": jj, "w": None, "nm": None})
        if sep in (2, 4):
            for r0, beta in R0_BETA_EXT:
                contact_jobs.append({"base": f"c_{sel}_s{sep}", "r0": r0, "beta": beta, "ii": ii, "jj": jj, "w": None, "nm": None})
    for (sel, sep), (ii, jj, w) in blists.items():
        for r0, beta in R0_BETA_RESBAL:
            contact_jobs.append({"base": f"cresb_{sel}_s{sep}", "r0": r0, "beta": beta, "ii": ii, "jj": jj, "w": w, "nm": None})
    for sel in RATIONAL_SELECTIONS:
        ii, jj = plists[(sel, 4)]
        for r0, nn, mm in RATIONAL_PARAMS:
            contact_jobs.append({"base": f"crat_{sel}_s4_n{nn}m{mm}", "r0": r0, "beta": None, "ii": ii, "jj": jj, "w": None, "nm": (nn, mm)})

    cols = {}
    for job in contact_jobs:
        cname = f"{job['base']}_r0{job['r0']:g}" + (f"_b{job['beta']:g}" if job["beta"] is not None else "")
        cols[cname] = np.zeros(n)
    ...
    base_names = ["rg_heavy", "rg_ca", "e2e_nc_ca", "e2e_n_c", "asphericity", "acylindricity",
    ...
    for fi, f in enumerate(files):
        ...
        for job in contact_jobs:
            ...
            if key not in dist_cache:
                dist_cache[key] = np.linalg.norm(pos_nm[job["ii"]] - pos_nm[job["jj"]], axis=1) * 10.0
            d = dist_cache[key]
    ...
    print("  torsion PCA explained var:", np.round(evr[:8], 3).tolist(), flush=True)

    key_cv = cols["c_bb_s4_r010_b3"]
    ...
    print("  residual torsion PCA explained var:", np.round(evrr[:8], 3).tolist(), flush=True)
    ...
    for name, atoms in (("burial_y2", [a.index for a in top.residue(1).atoms if a.name not in BB_NAMES]),
                        ("burial_w9", [a.index for a in top.residue(8).atoms if a.name not in BB_NAMES]),
                        ("burial_d3", [a.index for a in top.residue(2).atoms if a.name not in BB_NAMES])):
