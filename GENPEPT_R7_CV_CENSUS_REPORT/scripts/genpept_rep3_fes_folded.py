...
FIG = REPORT / "figures"
OUT_MD = REPORT / "REP3_FES_FOLDED.md"
OUT_JSON = REPORT / "genpept_rep3_fes_folded.json"

BANKS = {"r7": R7, "rep3_massive": R3}
...
def main():
    ...
    md_lines = ["# Folded-state proximity audit + rep3 pseudo-FES maps\n",
                f"Reference: PDB 1UAO (chignolin GYDPETGTWG NMR ensemble, {native.n_frames} models, "
                f"fetched from RCSB). Native pair distances over the ensemble: "
                f"d(D3 N, G7 O) {nat_d37.min():.2f}..{nat_d37.max():.2f} A; "
    ...
    for tag, root in BANKS.items():
        ...
        ax = axes[0, 0]
        ax.hist(rmsd_min, bins=80, color="#3182bd")
        ...
        if tag == "rep3_massive":
            ...
            fig2.suptitle(f"rep3_massive pseudo-FES maps ({n} seeds, search density - not equilibrium FES)", fontsize=11)
            fig2.tight_layout(rect=[0, 0, 1, 0.97])
            fig2.savefig(FIG / "cv_rep3massive_pseudofes.png", dpi=150)
            plt.close(fig2)
            occ = float((H > 0).mean())
            md_lines.append("\n## rep3 pseudo-FES")
            md_lines.append(f"- grid occupancy on CV1xPC1 map: {occ*100:.1f}%")
            md_lines.append("- figures: cv_rep3massive_pseudofes.png (3 maps), cv_rep3_massive_folded_proximity.png")
