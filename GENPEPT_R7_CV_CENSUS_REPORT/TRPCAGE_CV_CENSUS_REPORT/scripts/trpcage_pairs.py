...
FIG = REPORT / "figures"
OUT_MD = REPORT / "TRPCAGE_ORTHOGONAL_PAIRS.md"
OUT_JSON = REPORT / "trpcage_orthogonal_pairs.json"
OUT_TSVG = REPORT / "trpcage_orthogonal_pairs.csv"

CV1_CANDS = ["c_bb_s4_r010_b3", "c_ca_s4_r010_b3", "c_heavy_s4_r012_b3", "c_sch_s4_r012_b3"]
CV2_CANDS = ["resPC1_refit", "restorpca_pc1", "torpca_pc2", "torpca_pc1", "cage_score",
             "cage_w6_pros_r06_b4", "dssp_helix_1_9", "rama_alpha_1_9", "pack_w6",
             "burial_w6", "acylindricity", "shape_anisotropy", "chirality_ca",
             "e2e_nc_ca", "sasa_hydrophobic_frac", "contact_order_ca8", "rg_helix_1_9"]
NEGATIVE_CONTROLS = {"contact_order_ca8", "e2e_nc_ca", "d_w6_p12_ca"}
...
def main():
    ...
    fig.suptitle("top alternative orthogonal CV1 x CV2 pairs (trp-cage bank, hex counts)", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(FIG / "trpcage_orthogonal_pairs_top.png", dpi=150)
    plt.close(fig)

    lines = ["# Alternative orthogonal CV1 x CV2 pairs — scored on the trp-cage (TC5b) bank\n"]
    lines.append(f"n={n} seeds. Per pair: Pearson & Spearman r, 16x8 quantile-grid occupancy, min bin count,")
    ...
    lines.append("Composite score = min(ent1, ent2) x (1-|r_s|) x occupancy x frac_bins_IQR50.\n")
    ...
    print("wrote", OUT_MD, OUT_TSVG, OUT_JSON)
