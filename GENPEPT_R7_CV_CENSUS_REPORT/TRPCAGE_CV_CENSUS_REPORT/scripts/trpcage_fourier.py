...
OUT_MD = REPORT / "TRPCAGE_FOURIER.md"
OUT_JSON = REPORT / "trpcage_fourier.json"

SEQ = "NLYIQWLKDGGPSSGRPPPS"
RES3 = [f"{SEQ[i]}{i+1}" for i in range(len(SEQ))]
N_PERM = 300
...
def main():
    ...
    n, d = TH.shape
    print(f"angles {TH.shape}", flush=True)
    ...
    axes[1].set_xticklabels(labels, rotation=60, fontsize=6)
    ...
    fig.tight_layout()
    fig.savefig(FIG / "trpcage_fourier_harmonics_corrmatrix.png", dpi=150)
    plt.close(fig)
    ...
    orderings = {
        ...
        "cage_score": census["cage_score"].to_numpy(),
        "rg_heavy": census["rg_heavy"].to_numpy(),
        "energy": census["energy_kj_mol"].to_numpy(),
    }
    wind_out = {}
    ...
    for ax, (oname, res) in zip(axes.ravel(), wind_out.items()):
        ...
        ax.set_ylabel("|winding| (turns)")
        ax.set_title(f"winding along {oname}", fontsize=10)
        ax.legend(fontsize=8)
        ax.grid(alpha=.3, axis="y")
    fig.tight_layout()
    fig.savefig(FIG / "trpcage_fourier_winding.png", dpi=150)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIG / "trpcage_fourier_winding_traces.png", dpi=150)
    plt.close(fig)
    ...
    lines = ["# Fourier analysis of trp-cage (TC5b) seed-bank dihedrals\n"]
    lines.append(f"bank: {n} seeds; {d} backbone torsions (phi, psi per residue).")
