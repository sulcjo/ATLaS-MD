...
ROOT = BASE / "RUNS" / "NLYIQWLKDGGPSSGRPPPS_genpept"
REPORT_DIR = Path(__file__).resolve().parents[1]
SEED_DIR = str(ROOT / "final_implicit_survivor_seeds")
OUT_CSV = str(REPORT_DIR / "trpcage_cv_census_values.csv")

SEQ = "NLYIQWLKDGGPSSGRPPPS"
W6, P12, P17, P18, P19 = 5, 11, 16, 17, 18
HELIX = (0, 8)
...
CAGE_PRO_R0_BETA = [(6.0, 4.0), (6.0, 6.0), (7.0, 4.0)]

BB_NAMES = {"N", "CA", "C", "O", "OXT"}
BB_H_NAMES = {"H", "H1", "H2", "H3", "HA", "HA2", "HA3"}
W_RING = ("CG", "CD1", "CD2", "NE1", "CE2", "CE3", "CZ2", "CZ3", "CH2")
...
def main():
    ...
    hb_helix_pairs = [(n_idx[i], o_idx[j], i, j) for i, j in hb_res
                      if HELIX[0] <= i <= HELIX[1] and HELIX[0] <= j <= HELIX[1]]

    w6_ring = [a.index for a in top.residue(W6).atoms if a.name in W_RING]
    w6_side = [a.index for a in top.residue(W6).atoms if a.name not in BB_NAMES]
    y3_side = [a.index for a in top.residue(2).atoms if a.name not in BB_NAMES]
    cage_pros = {"p12": P12, "p17": P17, "p18": P18, "p19": P19}
    ...
    pro_sc = {}
    for tag, ri in cage_pros.items():
        pro_sc[tag] = np.array([a.index for a in top.residue(ri).atoms
                                if a.name not in BB_NAMES
                                and (a.element is not None and a.element.symbol != "H")])
    w6_ring_arr = np.array(w6_ring)
    for tag in cage_pros:
        cols[f"cage_w6_{tag}"] = np.zeros(n)
        cols[f"cage_w6_{tag}_mindist"] = np.zeros(n)
    for r0, beta in CAGE_PRO_R0_BETA:
        cols[f"cage_w6_pros_r0{r0:g}_b{beta:g}"] = np.zeros(n)
    cols["pack_w6"] = np.zeros(n)
    cols["pack_y3"] = np.zeros(n)

    base_names = [
        "rg_heavy", "rg_ca", "e2e_nc_ca", "e2e_n_c", "asphericity", "acylindricity",
        "shape_anisotropy", "max_ca_ca", "mean_ca_ca", "helix_1_9_ca_span", "rg_helix_1_9",
        "d_w6_p12_ca", "d_w6_p18_ca", "d_y3_k8_ca", "d_w6_cage_centroid",
        "hbond_bb_ge3", "hbond_i_i3", "hbond_i_i4", "hbond_helix_window", "contact_order_ca8",
        "chirality_ca", "trpcage_like", "cage_score",
    ]
    ...
    for fi, f in enumerate(files):
        ...
        dmin = {}
        for tag, sc in pro_sc.items():
            dp = cdist(pos_nm[w6_ring_arr], pos_nm[sc]) * 10.0
            dmin[tag] = float(dp.min())
            cols[f"cage_w6_{tag}_mindist"][fi] = dmin[tag]
            cols[f"cage_w6_{tag}"][fi] = float(switch_fraction(dp.min(), 6.0, 4.0))
        for r0, beta in CAGE_PRO_R0_BETA:
            cols[f"cage_w6_pros_r0{r0:g}_b{beta:g}"][fi] = float(
                np.mean([switch_fraction(dmin[t], r0, beta) for t in cage_pros]))
        base["cage_score"][fi] = float(
            np.mean([switch_fraction(dmin[t], 6.0, 4.0) for t in cage_pros]))
        base["d_w6_cage_centroid"][fi] = float(np.linalg.norm(
            pos_nm[w6_ring_arr].mean(axis=0)
            - np.mean([pos_nm[pro_sc[t]].mean(axis=0) for t in cage_pros], axis=0)) * 10.0)

        for tag, (sc, res_i) in (("pack_w6", (np.array([a.index for a in top.residue(W6).atoms
                                                        if a.name not in BB_NAMES
                                                        and (a.element is not None and a.element.symbol != "H")]), W6)),
                                 ("pack_y3", (np.array([a.index for a in top.residue(2).atoms
                                                         if a.name not in BB_NAMES
                                                         and (a.element is not None and a.element.symbol != "H")]), 2))):
            others = heavy_arr_local = np.array([a.index for a in top.atoms
                                                 if a.residue.index != res_i and a.element is not None
        ...
        base["d_w6_p12_ca"][fi] = dca[W6, P12]
        base["d_w6_p18_ca"][fi] = dca[W6, P18]
        base["d_y3_k8_ca"][fi] = dca[2, 7]
        helix_ca = ca_idx[HELIX[0]:HELIX[1] + 1]
        dh = cdist(pos_nm[helix_ca], pos_nm[helix_ca]) * 10.0
        base["helix_1_9_ca_span"][fi] = dh.max()
        base["rg_helix_1_9"][fi] = math.sqrt(gyration_eigs(pos_nm, helix_ca).sum())

        nb_hb = nb_hb3 = nb_hb4 = nb_hbh = 0.0
        for a, b, i, j in hb_all_pairs:
            ...
            if abs(i - j) == 4:
                nb_hb4 += v
        for a, b, i, j in hb_helix_pairs:
            d = np.linalg.norm(pos_nm[a] - pos_nm[b]) * 10.0
            nb_hbh += switch_fraction(d, 3.5, 8.0)
        base["hbond_bb_ge3"][fi] = nb_hb
        base["hbond_i_i3"][fi] = nb_hb3
        base["hbond_i_i4"][fi] = nb_hb4
        base["hbond_helix_window"][fi] = nb_hbh
        ...
        m = np.stack([pos_nm[ca_idx[6]] - pos_nm[ca_idx[1]],
                      pos_nm[ca_idx[5]] - pos_nm[ca_idx[1]],
                      pos_nm[ca_idx[8]] - pos_nm[ca_idx[1]]])
    ...
    dssp = md.compute_dssp(alltraj, simplified=True)

    dssp_helix = (dssp == "H").sum(axis=1)
    dssp_helix_1_9 = (dssp[:, HELIX[0]:HELIX[1] + 1] == "H").sum(axis=1)
    cols["dssp_helix_total"] = dssp_helix.astype(float)
    cols["dssp_helix_1_9"] = dssp_helix_1_9.astype(float)

    for k, v in base.items():
        cols[k] = v

    cage_like = ((cols["cage_w6_p12"] > 0.5) & (cols["cage_w6_p17"] > 0.5)
                 & (cols["cage_w6_p18"] > 0.5) & (cols["cage_w6_p19"] > 0.5)
                 & (dssp_helix_1_9 >= 5)).astype(float)
    cols["trpcage_like"] = cage_like
    ...
    n_ppii = np.zeros(n)
    n_left = np.zeros(n)
    alpha_1_9 = np.zeros(n)
    res_of_phi = [int(top.atom(int(i[1])).residue.index) for i in phi_idx]
    for k in range(phis.shape[1]):
        p, q = phi_deg[:, k], psi_deg[:, k]
        a = (((-100 <= p) & (p <= -20)) & ((-80 <= q) & (q <= -10))).astype(float)
        b = (((-180 <= p) & (p <= -100)) & ((80 <= q) & (q <= 180))).astype(float)
        p3 = (((-100 <= p) & (p <= -40)) & ((120 <= q) & (q <= 180))).astype(float)
        lf = (((20 <= p) & (p <= 110)) & ((-10 <= q) & (q <= 100))).astype(float)
        n_alpha += a
        n_beta += b
        n_ppii += p3
        n_left += lf
        if HELIX[0] <= res_of_phi[k] <= HELIX[1]:
            alpha_1_9 += a
    cols["rama_alpha"] = n_alpha
    cols["rama_beta"] = n_beta
    cols["rama_ppii"] = n_ppii
    cols["rama_left"] = n_left
    cols["rama_alpha_1_9"] = alpha_1_9
    ...
    for res_i, tag in ((W6, "w6"), (2, "y3"), (15, "r16")):
        if res_i in chi_res:
    ...
    for name, atoms in (("burial_w6", w6_side), ("burial_y3", y3_side)):
    ...
    surv = pd.read_csv(ROOT / "final_survivor_seeds.csv")
    base2row = {Path(p).name: i for i, p in enumerate(surv["survivor_pdb_path"])}
    row_of = np.array([base2row[f + ".pdb"] for f in names])
    cols["energy_kj_mol"] = surv["minimized_energy_kj_mol"].to_numpy()[row_of]
